...
class Compose(object):
    ...
    def __call__(self, image, label, link):
        for t in self.segtransform:
            image, label, link = t(image, label, link)
...
class ToTensor(object):
    ...
    def __call__(self, image, label, link):
        if not isinstance(image, np.ndarray) or not isinstance(label, np.ndarray) or not isinstance(link, np.ndarray):
        ...
        if not isinstance(label, torch.LongTensor):
            label = label.long()
        link = torch.from_numpy(link).unsqueeze(dim=-1)
        return image, label, link
...
class Resize(object):
    ...
    def __call__(self, image, label, link):
        ...
        link[:, 1] = np.round((link[:, 1]) * h2 / h1).astype(np.int).clip(0, h2 - 1)
        link[:, 2] = np.round((link[:, 2]) * w2 / w1).astype(np.int).clip(0, w2 - 1)
        return image, label, link
...
class RandScale(object):
    ...
    def __call__(self, image, label, link):
        ...
        link[:, 1] = np.round(link[:, 1] * scale_factor_x).astype(np.int)
        link[:, 2] = np.round(link[:, 2] * scale_factor_y).astype(np.int)
        return image, label, link
...
class Crop(object):
    ...
    def __call__(self, image, label, link):
        ...
        link[:, 1] = link[:, 1] - h_off
        link[:, 2] = link[:, 2] - w_off
        link[:, 3][link[:, 1] >= self.crop_h] = 0
        link[:, 3][link[:, 2] >= self.crop_w] = 0
        link[:, 3][link[:, 1] < 0] = 0
        link[:, 3][link[:, 2] < 0] = 0
        return image, label, link
